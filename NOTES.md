...
- Description: **WITHDRAWN.** `Where you stand and what you see.` was proposed;
  Noah, 2026-07-30: *"That description sucks."* He is right about the job it was
  failing to do — that line is the tagline, and it already appears in `<title>`,
  `og:title`, `twitter:title`, the About panel and the README's first line,
  while the repo description is read in search results and in a repo list where
  the only question is *what is this*. Candidates, functional first:
  - `Free perspective drawing — set vanishing points, draw lines that stay locked to them. Offline, no account.`
  - `A free drawing tool for perspective construction: vanishing points, constrained lines, boxes in one drag.`
  - `Perspective drawing for iPad and desktop. Vanishing points that hold your lines. Free, offline, no account.`
  Awaiting Noah's pick; nothing is set.
...
- Social preview: **ready now** — `public/og.png` (1200×630). Upload it in the
  ...
  and one plain line. Regenerated by `npm run render:og`
  ([`render-og.mjs`](render-og.mjs)) from `art/og-base.png`, which is the
